...
export default function SettingsScreen({ settings, onSave, activeProfile, updateProfile, addProfile, removeProfile, onClose, user, onSignOut, onShowSources, calendar }) {
  ...
  const save = () => {
    onSave({ city, homeAddress, intervalMinutes:interval, testMode, ambientTimeoutMinutes:ambientMins, spotlightMode, columnOrder, curatedMode });
    onClose();
  };
  ...
  return (
    <div className="fade-enter" style={{ position:'fixed', inset:0, background:'#141210', zIndex:60, overflowY:'auto', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ padding:16, flex:1 }}>
        ...
        <Section title="Google Calendar" desc="Calendar access is granted automatically when you sign in with Google">
          <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
            <span style={{ fontSize:11, padding:'3px 9px', borderRadius:99,
              background: calendar?.connected ? 'rgba(26,99,50,.3)' : 'rgba(255,255,255,.06)',
              color:      calendar?.connected ? '#6EE7A0'           : 'rgba(255,255,255,.4)',
              border:     calendar?.connected ? '0.5px solid rgba(110,231,160,.25)' : '0.5px solid rgba(255,255,255,.1)',
            }}>
              {calendar?.connected
                ? `✓ Connected${calendar.email ? ` · ${calendar.email}` : ''}`
                : user ? 'Connecting…' : 'Sign in with Google to connect'}
            </span>
            {calendar?.connected && (
              <button onClick={() => calendar.disconnect?.()} style={{ fontSize:11, padding:'4px 11px', borderRadius:8, cursor:'pointer', background:'rgba(255,255,255,.07)', border:'0.5px solid rgba(255,255,255,.12)', color:'rgba(255,255,255,.4)', fontFamily:'DM Sans, sans-serif' }}>
                Disconnect
              </button>
            )}
          </div>
          <div style={{ fontSize:10, color:'rgba(255,255,255,.25)', marginTop:5 }}>Events you add via "Add to calendar" will appear in Google Calendar.</div>
        </Section>
        ...
        {/* Sources page intentionally hidden from main UI — use /admin routes directly */}
