...
function WeekendSidebar({ activities, calQueue, weather, onCal, onWeather }) {
// Top spotlight event
const hero = Object.values(activities).flat()
.filter(a => a?.title)
...
const calConnected = false; // TODO: wire from settings.gcalConnected
  ...
  <div style={{flex:1,overflowY:'auto'}} className="no-scroll">
    ...
    {!calConnected && (
      <div style={{margin:'8px',padding:'10px 12px',borderRadius:8,background:'rgba(201,168,76,.07)',border:'0.5px solid rgba(201,168,76,.15)',cursor:'pointer'}}
        onClick={() => {}}>
        <div style={{fontSize:10,color:'#C9A84C',fontWeight:600,marginBottom:3}}>Connect Google Calendar</div>
        <div style={{fontSize:9,color:'rgba(255,255,255,.3)',lineHeight:1.4}}>See what you already have planned alongside event suggestions</div>
      </div>
    )}
  </div>
</div>
